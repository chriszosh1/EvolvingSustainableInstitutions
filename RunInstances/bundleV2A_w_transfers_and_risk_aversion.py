...
'''
   ...
   eg. suppose fine_vector = [0, 2, 4, 6, 8, 10],
    ...
    This Scenario now considers agents which now have risk
    averse utility functions, as opposed to being risk 
    neutral as before. Let's see how this, if anything, changes
    the 'optimal' policy chosen by the social planner.
'''
...
av = {'vision':1, 'action_set':{'sheep_count_if_seen': {'min': 0, 'max': 5, 'grain':1}},
      'utility_fnc': risk_averse_utility} #<-- RA arg
#Output params:
vv = []
dto = {'tag': '', 'files':['final']}
...
def main():
    run_model_bundle(gv = gv, av=av, vv = vv, dto = dto, sv = sv, ils_sv = ils_sv,
                     pv = pv, ffa = ffa, tag = 'Bundle_w_transfers_and_risk_aversion', finetune=False)
